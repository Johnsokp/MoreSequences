...
def run_test_generate_points_on_circle():
    ...
    # Test 2:
    expected = [rg.Point(125, 25),  # All numbers are approximate.
                rg.Point(75, 75),
                rg.Point(25, 25),
                rg.Point(75, -25)]
    circle = rg.Circle(rg.Point(75, 25), 50)
...
def run_test_draw_points_on_circle():
    ...
    # Test 4:
    title = 'DRAW_POINTS_ON_CIRCLE, test 4:  21 red dots.'
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(272, 286), 111)
    draw_points_on_circle(window, circle, 21, 'red')
    window.close_on_mouse_click()
...
def run_test_pizza():
    ...
    pizza(window, circle, 272, 'black', 1)
    window.close_on_mouse_click()
...
def run_test_polygon():
    ...
    polygon(window, circle, 3, 'black', 5)
...
def polygon(window, circle, number_of_segments, color, thickness):
    ...
    for k in range(len(sequence)):
        point = sequence[k]
        line = rg.Line(point, sequence[(k + 1) % len(sequence)])
        line.color = color
        line.thickness = thickness
        line.attach_to(window)
        window.render()
...
def run_test_fancy_polygon():
    ...
    fancy_polygon(window, circle, 300, 100, 'blue', 1)
    window.close_on_mouse_click()
...
def fancy_polygon(window, circle, number_of_lines, hops_to_next_point, color, thickness):
    ...
    for k in range(len(sequence)):
        point = sequence[k]
        line = rg.Line(point, sequence[(k + hops_to_next_point) % len(sequence)])
        line.color = color
        line.thickness = thickness
        line.attach_to(window)
        window.render()
